/*******************************************************************************
* File Name: I2C_MASTER_I2C_BOOT.c
* Version 2.0
*
* Description:
...
static uint8 I2C_MASTER_slReadBuf[I2C_MASTER_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 I2C_MASTER_slWriteBuf[I2C_MASTER_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 I2C_MASTER_applyBuffer;
...
static void I2C_MASTER_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: I2C_MASTER_I2CCyBtldrCommStart
********************************************************************************
*
* Summary:
...
{
    I2C_MASTER_I2CSlaveInitWriteBuf(I2C_MASTER_slWriteBuf, I2C_MASTER_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_MASTER_I2CSlaveInitReadBuf (I2C_MASTER_slReadBuf, 0u);

    I2C_MASTER_SetCustomInterruptHandler(&I2C_MASTER_I2CResposeInsert);
    I2C_MASTER_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CCyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CCyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CCyBtldrCommRead
********************************************************************************
*
* Summary:
...
cystatus I2C_MASTER_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)
        {
            ...
            if(0u != (I2C_MASTER_I2C_SSTAT_WR_CMPLT & I2C_MASTER_slStatus))
            {
                /* Copy command into bootloader buffer */
                (void) memcpy((void *) pData, (const void *) I2C_MASTER_slWriteBuf,
                                                I2C_MASTER_I2C_MIN_UINT16(I2C_MASTER_slWrBufIndex, size));

                /* Copy number of written bytes */
                *count = (uint16) I2C_MASTER_slWrBufIndex;
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CCyBtldrCommWrite
********************************************************************************
*
* Summary:
...
cystatus I2C_MASTER_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        (void) memcpy((void *) I2C_MASTER_slReadBuf, (const void *) pData, (uint32) size);
        *count = size; /* Buffer was copied to I2C buffer */

        /* Read buffer is ready to be released to host */
        I2C_MASTER_applyBuffer = (uint32) size;

        while(0u != timeoutMs)
        {
            /* Check if response has been read */
            if(I2C_MASTER_slRdBufIndex == (uint32) size)
            {
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CResposeInsert
********************************************************************************
*
* Summary:
...
*  I2C_MASTER_slRdBufIndex - used to store current index within slave
*  read buffer.
*  I2C_MASTER_applyBuffer - flag to release buffer with response to be
*  read
*
*******************************************************************************/
static void I2C_MASTER_I2CResposeInsert(void)
{
    ...
    {
        if(0u != I2C_MASTER_applyBuffer)
        {
            /* Response was copied into buffer: release buffer to host */
            I2C_MASTER_slRdBufSize = I2C_MASTER_applyBuffer;
            I2C_MASTER_applyBuffer = 0u;
        }
    }
}
