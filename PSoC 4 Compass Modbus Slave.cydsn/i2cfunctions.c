...
void I2CReadBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *value) {
	uint8_t i=0;
	I2C_MASTER_I2CMasterSendStart(devAddr, I2C_MASTER_I2C_WRITE_XFER_MODE);
	I2C_MASTER_I2CMasterWriteByte(regAddr);
	I2C_MASTER_I2CMasterSendRestart(devAddr, I2C_MASTER_I2C_READ_XFER_MODE);
	while (i++ < (length-1)) {
		*value++ = I2C_MASTER_I2CMasterReadByte(I2C_MASTER_I2C_ACK_DATA);
	}
	*value = I2C_MASTER_I2CMasterReadByte(I2C_MASTER_I2C_NAK_DATA);
...
void I2CWriteBytes(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint8_t *value) {
	...
	I2C_MASTER_I2CMasterWriteByte(regAddr);
	while (i++ < length) {
		I2C_MASTER_I2CMasterWriteByte(*value++);
	}
	I2C_MASTER_I2CMasterSendStop();	
}
...
void I2CWriteWords(uint8_t devAddr, uint8_t regAddr, uint8_t length, uint16_t *value) {
	...
	I2C_MASTER_I2CMasterWriteByte(regAddr);
	while (i++ < length) {
		I2C_MASTER_I2CMasterWriteByte(((uint8_t)*value) >> 8);
		I2C_MASTER_I2CMasterWriteByte((uint8_t)*value++);
	}
	I2C_MASTER_I2CMasterSendStop();		
}
