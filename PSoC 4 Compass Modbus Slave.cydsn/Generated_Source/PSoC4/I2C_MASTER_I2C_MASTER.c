/*******************************************************************************
* File Name: I2C_MASTER_I2C_MASTER.c
* Version 2.0
*
* Description:
...
/* Master variables */
volatile uint16 I2C_MASTER_mstrStatus;      /* Master Status byte  */
...
volatile uint32  I2C_MASTER_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  I2C_MASTER_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * I2C_MASTER_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  I2C_MASTER_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  I2C_MASTER_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  I2C_MASTER_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!I2C_MASTER_CY_SCBIP_V0 && \
    I2C_MASTER_I2C_MULTI_MASTER_SLAVE_CONST && I2C_MASTER_I2C_WAKE_ENABLE_CONST)
    static void I2C_MASTER_I2CMasterDisableEcAm(void);
#endif /* (!I2C_MASTER_CY_SCBIP_V0) */


/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterWriteBuf
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = I2C_MASTER_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(I2C_MASTER_CHECK_I2C_FSM_IDLE)
        {
            I2C_MASTER_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = I2C_MASTER_CHECK_I2C_STATUS(I2C_MASTER_I2C_STATUS_BUS_BUSY) ?
                            I2C_MASTER_I2C_MSTR_BUS_BUSY : I2C_MASTER_I2C_MSTR_NO_ERROR;
        }
        else if(I2C_MASTER_CHECK_I2C_FSM_HALT)
        {
            I2C_MASTER_mstrStatus &= (uint16) ~I2C_MASTER_I2C_MSTAT_XFER_HALT;
                              errStatus  = I2C_MASTER_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(I2C_MASTER_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
    #if (!I2C_MASTER_CY_SCBIP_V0 && \
        I2C_MASTER_I2C_MULTI_MASTER_SLAVE_CONST && I2C_MASTER_I2C_WAKE_ENABLE_CONST)
            I2C_MASTER_I2CMasterDisableEcAm();
    #endif /* (!I2C_MASTER_CY_SCBIP_V0) */

        /* Set up write transaction */
        I2C_MASTER_state = I2C_MASTER_I2C_FSM_MSTR_WR_ADDR;
        I2C_MASTER_mstrWrBufIndexTmp = 0u;
        I2C_MASTER_mstrWrBufIndex    = 0u;
        I2C_MASTER_mstrWrBufSize     = cnt;
        I2C_MASTER_mstrWrBufPtr      = (volatile uint8 *) wrData;
        I2C_MASTER_mstrControl       = (uint8) mode;

        slaveAddress = I2C_MASTER_GET_I2C_8BIT_ADDRESS(slaveAddress);

        I2C_MASTER_mstrStatus &= (uint16) ~I2C_MASTER_I2C_MSTAT_WR_CMPLT;
        ...
        if(I2C_MASTER_CHECK_I2C_MODE_RESTART(mode))
        {
            I2C_MASTER_I2C_MASTER_GENERATE_RESTART;
            I2C_MASTER_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            I2C_MASTER_TX_FIFO_WR_REG = slaveAddress;
            I2C_MASTER_I2C_MASTER_GENERATE_START;
        }
    ...
    I2C_MASTER_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterReadBuf
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(I2C_MASTER_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        I2C_MASTER_state = I2C_MASTER_I2C_FSM_MSTR_RD_ADDR;
        I2C_MASTER_mstrRdBufIndex = 0u;
        I2C_MASTER_mstrRdBufSize  = cnt;
        I2C_MASTER_mstrRdBufPtr   = (volatile uint8 *) rdData;
        I2C_MASTER_mstrControl    = (uint8) mode;

        slaveAddress = (I2C_MASTER_GET_I2C_8BIT_ADDRESS(slaveAddress) | I2C_MASTER_I2C_READ_FLAG);

        I2C_MASTER_mstrStatus &= (uint16) ~I2C_MASTER_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(I2C_MASTER_mstrRdBufSize < I2C_MASTER_I2C_FIFO_SIZE) /* Reading byte-by-byte */
        {
            I2C_MASTER_SetRxInterruptMode(I2C_MASTER_INTR_RX_NOT_EMPTY);
        }
        else /* Receive RX FIFO chunks */
        {
            I2C_MASTER_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterSendStart
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(I2C_MASTER_CHECK_I2C_FSM_IDLE)
    {
        ...
        if(I2C_MASTER_CHECK_I2C_STATUS(I2C_MASTER_I2C_STATUS_BUS_BUSY))
        {
            errStatus = I2C_MASTER_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            I2C_MASTER_DisableInt();  /* Lock from interruption */
            ...
            else /* Read direction */
            {
                I2C_MASTER_state = I2C_MASTER_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= I2C_MASTER_I2C_READ_FLAG;
            }

            /* TX and RX FIFO have to be EMPTY */

            I2C_MASTER_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            I2C_MASTER_ClearMasterInterruptSource(I2C_MASTER_INTR_MASTER_ALL);

            I2C_MASTER_I2C_MASTER_GENERATE_START;


            while(!I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_ACK      |
                                                      I2C_MASTER_INTR_MASTER_I2C_NACK     |
                                                      I2C_MASTER_INTR_MASTER_I2C_ARB_LOST |
            ...
            if(I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_ACK))
            {
                errStatus = I2C_MASTER_I2C_MSTR_NO_ERROR;
            }
            else if(I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_NACK))
            {
                errStatus = I2C_MASTER_I2C_MSTR_ERR_LB_NAK;
            }
            else if(I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_ARB_LOST))
            {
                I2C_MASTER_state = I2C_MASTER_I2C_FSM_IDLE;
                             errStatus = I2C_MASTER_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = I2C_MASTER_I2C_RESET_ERROR;
            }
            else /* I2C_MASTER_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                I2C_MASTER_state = I2C_MASTER_I2C_FSM_IDLE;
                             errStatus = I2C_MASTER_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = I2C_MASTER_I2C_RESET_ERROR;
            }

            I2C_MASTER_ClearMasterInterruptSource(I2C_MASTER_INTR_MASTER_I2C_ACK      |
                                                        I2C_MASTER_INTR_MASTER_I2C_NACK     |
                                                        I2C_MASTER_INTR_MASTER_I2C_ARB_LOST |
                                                        I2C_MASTER_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterSendRestart
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(I2C_MASTER_CHECK_I2C_MASTER_ACTIVE)
    {
        slaveAddress = I2C_MASTER_GET_I2C_8BIT_ADDRESS(slaveAddress);
        ...
        */
        ...
        while(I2C_MASTER_CHECK_I2C_MASTER_CMD(I2C_MASTER_I2C_MASTER_CMD_M_START))
        {
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterSendStop
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterSendStop(void)
{
    ...
    if(I2C_MASTER_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_STOP     |
                                                  I2C_MASTER_INTR_MASTER_I2C_ARB_LOST |
                                                  I2C_MASTER_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_STOP))
        {
            errStatus = I2C_MASTER_I2C_MSTR_NO_ERROR;
        }
        else if(I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* I2C_MASTER_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = I2C_MASTER_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = I2C_MASTER_I2C_RESET_ERROR;
        }
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterWriteByte
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(I2C_MASTER_CHECK_I2C_MASTER_ACTIVE)
    {
        I2C_MASTER_TX_FIFO_WR_REG = theByte;
        ...
        else /* I2C_MASTER_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            I2C_MASTER_state = I2C_MASTER_I2C_FSM_IDLE;
                         errStatus = I2C_MASTER_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = I2C_MASTER_I2C_RESET_ERROR;
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterReadByte
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = I2C_MASTER_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(I2C_MASTER_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!I2C_MASTER_CHECK_INTR_RX(I2C_MASTER_INTR_RX_NOT_EMPTY)) &&
              (!I2C_MASTER_CHECK_INTR_MASTER(I2C_MASTER_INTR_MASTER_I2C_ARB_LOST |
                                                  I2C_MASTER_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(I2C_MASTER_CHECK_INTR_RX(I2C_MASTER_INTR_RX_NOT_EMPTY))
        {
            theByte = I2C_MASTER_RX_FIFO_RD_REG;
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterGetReadBufSize
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterGetWriteBufSize
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterClearWriteBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterStatus
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterStatus(void)
{
    ...
    status = (uint32) I2C_MASTER_mstrStatus;

    if (I2C_MASTER_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) I2C_MASTER_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CMasterClearStatus
********************************************************************************
*
* Summary:
...
uint32 I2C_MASTER_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) I2C_MASTER_mstrStatus;
    I2C_MASTER_mstrStatus = I2C_MASTER_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: I2C_MASTER_I2CReStartGeneration
********************************************************************************
*
* Summary:
...
void I2C_MASTER_I2CReStartGeneration(void)
{
    ...
    uint32 status = I2C_MASTER_I2C_STATUS_REG;

    while(I2C_MASTER_WAIT_SDA_SET_HIGH)
    {
        /* Wait when slave release SDA line: SCL tHIGH is complete */
    }

    /* Prepare DR register to drive SCL line */
    I2C_MASTER_SET_I2C_SCL_DR(I2C_MASTER_I2C_SCL_LOW);

    /* Switch HSIOM to GPIO: SCL goes low */
    I2C_MASTER_SET_I2C_SCL_HSIOM_SEL(I2C_MASTER_HSIOM_GPIO_SEL);

    /* Disable SCB block */
    I2C_MASTER_CTRL_REG &= (uint32) ~I2C_MASTER_CTRL_ENABLED;

    if(0u != (status & I2C_MASTER_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        I2C_MASTER_SET_I2C_SCL_DR(I2C_MASTER_I2C_SCL_LOW);
        CyDelayUs(I2C_MASTER_I2C_TLOW_TIME); /* Count tLOW */

        I2C_MASTER_SET_I2C_SCL_DR(I2C_MASTER_I2C_SCL_HIGH);
        while(I2C_MASTER_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(I2C_MASTER_I2C_THIGH_TIME); /* Count tHIGH */
    }
    ...
    I2C_MASTER_I2C_MASTER_CMD_REG = I2C_MASTER_I2C_MASTER_CMD_M_START;

    /* Enable SCB block */
    I2C_MASTER_CTRL_REG |= (uint32) I2C_MASTER_CTRL_ENABLED;

    /* Switch HSIOM to I2C: */
    I2C_MASTER_SET_I2C_SCL_HSIOM_SEL(I2C_MASTER_HSIOM_I2C_SEL);
    ...
    cmd  = I2C_MASTER_I2C_MASTER_CMD_M_START;
    cmd |= I2C_MASTER_CHECK_I2C_STATUS(I2C_MASTER_I2C_STATUS_M_READ) ?
                (I2C_MASTER_I2C_MASTER_CMD_M_NACK) : (0u);

    I2C_MASTER_I2C_MASTER_CMD_REG = cmd;
    ...
    I2C_MASTER_I2C_MULTI_MASTER_SLAVE_CONST && I2C_MASTER_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: I2C_MASTER_I2CMasterDisableEcAm
    ********************************************************************************
    *
    * Summary:
    ...
    static void I2C_MASTER_I2CMasterDisableEcAm(void)
    {
        ...
        * This applicable only for Multi-Master-Slave with wakeup enabled. Ticket ID#192742 */
        if (0u != (I2C_MASTER_CTRL_REG & I2C_MASTER_CTRL_EC_AM_MODE))
        {
            ...
            I2C_MASTER_Stop();
            I2C_MASTER_CTRL_REG &= (uint32) ~I2C_MASTER_CTRL_EC_AM_MODE;
            I2C_MASTER_Enable();
        }
    }
#endif /* (!I2C_MASTER_CY_SCBIP_V0) */
